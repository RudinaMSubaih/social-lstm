...
from helper import get_all_file_names, delete_file, create_directories, remove_file_extention, add_file_extention, \
	rotate, vectorize_seq
...
class data_augmentator():
    ...
    def __init__(self, f_prefix, num_of_data, seq_length, val_percent):
        ...
        print("For each dataset -----> Number of additional training dataset: ", self.num_train_data,
              " Number of validation dataset: ", self.num_validation_data)
    ...
    def random_dataset_creation(self):
        ...
        for batch in range(self.dataloader.num_batches):
            ...
            for index, angle in enumerate(random_angles):
                ...
                modified_x_seq = self.submision_seq_preprocess(self.handle_seq(x_seq, lookup_seq, PedsList_seq, angle),
                                                               self.seq_length, lookup_seq)
            ...
            print('Current file : ', file_name, ' Processed trajectory number : ', batch + 1, 'out of',
                  self.dataloader.num_batches, 'trajectories in time', end - start)

        # write modified datapoints to txt files
        whole_dataset.append(dataset_instances)
        create_directories(os.path.join(self.f_prefix, self.base_validation_path),
                           self.dataloader.get_all_directory_namelist())
    ...
    def handle_seq(self, x_seq, lookup_seq, PedsList_seq, angle):
        ...
        for ind, frame in enumerate(vectorized_x_seq):
            for ped in PedsList_seq[ind]:
                ...
                modified_x_seq[ind, lookup_seq[ped], 0] = (rotate(origin, first_values_dict[ped], math.radians(angle)))[
                                                              0] + modified_x_seq[ind, lookup_seq[ped], 0]
                modified_x_seq[ind, lookup_seq[ped], 1] = (rotate(origin, first_values_dict[ped], math.radians(angle)))[
                                                              1] + modified_x_seq[ind, lookup_seq[ped], 1]
