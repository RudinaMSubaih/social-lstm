...
def get_parser_args():
    ...
    # number of iteration -> we are trying many times to get lowest test error derived from observed part and
    # prediction of observed part.Currently it is useless because we are using direct copy of observed part and no
    # use of prediction.Test error will be 0.
...
if __name__ == '__main__':
    ...
    for iteration in range(sample_args.iteration):
        ...
        for batch in range(dataloader.num_batches):
            ...
            # 0].data.numpy(), x_seq[1][lookup_seq[target_id], 1].data.numpy())) x_seq = rotate_traj_with_target_ped(
            # x_seq, angle, PedsList_seq, lookup_seq) grid_seq = getSequenceGridMask(x_seq[:sample_args.obs_length],
            # dataset_data, PedsList_seq, saved_args.neighborhood_size, saved_args.grid_size, sample_args.use_cuda)
            # x_seq, first_values_dict = vectorize_seq(x_seq, PedsList_seq, lookup_seq)
