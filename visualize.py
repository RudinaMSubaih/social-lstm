...
import argparse
from helper import get_all_file_names, vectorize_seq, rotate \
    , remove_file_extention, clear_folder, translate, get_method_name
...
def annotate_plot(plt, x_vals, y_vals, annotation):
    ...
    adjust_text(texts, force_points=1.5, expand_points=(3, 3),
                arrowprops=dict(arrowstyle="->", color='b', alpha=0.5, lw=0.5))
...
def plot_trajectories(true_trajs, pred_trajs, nodesPresent, look_up, frames, name, plot_directory, min_threshold,
    ...
    # angle_true = angle_between(reference_point, (true_trajs[target_frame][look_up[target_id], 0], true_trajs[
    # target_frame][look_up[target_id], 1])) angle_pred = angle_between(reference_point, (pred_trajs[target_frame][
    # look_up[target_id], 0], pred_trajs[target_frame][look_up[target_id], 1]))
    ...
    for j in shuffled_ped_ids:
        ...
        if not len(true_x) > min_threshold or not len(
            ...
            print(
                "Ped processing is aborted because its trajectory lenght in this sequence is smaller than threshold "
                "or 1 points")
            continue
        else:
        ...
        filtered_pred_x = min_max_scaler(np.array([x for x in pred_x if not np.isnan(x)]), overaall_min_x,
                                         overaall_max_x, min_r, max_r)
    ...
    plt.gcf().clear()
    # video animation
    create_plot_animation(plt, video_plot_trajs, processed_ped_index, target_id, real_inv_lookup, obs_length, markers,
...
def create_plot_animation(plt, trajs, shuffled_ped_ids, target_id, inv_lookup, seq_lenght, marker, colors, name,
                          data_frames, true_target_id_values, plot_directory, style, num_of_color):
    ...
    for frame_num in range(0, seq_lenght - 1):
        ...
        for elem_index, index in enumerate(shuffled_ped_ids):
            ...
            if ped_id == target_id:
                ...
                pred_ped_text = 'target ped ' + str(ped_id) + ' pred.'
                if frame_num == 0:
                    pred_sc = plt.scatter(filtered_pred_x[0:real_pred_index + 1],
                                          filtered_pred_y[0:real_pred_index + 1], marker=marker_arr[elem_index],
                                          label=pred_ped_text)
            ...
            if frame_num == 0:
                true_sc = plt.scatter(filtered_true_x[0:real_true_index + 1], filtered_true_y[0:real_true_index + 1],
                                      marker=marker_arr[elem_index], label=true_ped_text)

            else:
                true_sc = plt.scatter(filtered_true_x[0:real_true_index + 1], filtered_true_y[0:real_true_index + 1],
                                      marker=marker_arr[elem_index])
...
def plot_target_trajs(trajs, plot_directory, num_of_traj, plot_offset):
    ...
    for arr_index, index in enumerate(biggest_err_index):
        ...
        seq_str = seq_str + 'R: ' + str(arr_index + 1) + ' S: ' + str(
            index) + ' ME: ' + f"{mean_err:.{precision}}" + ' FE: ' + f"{final_err:.{precision}}" + ' | '
        ...
        print("**************************")
        true_line = plt.plot(true_traj[0], true_traj[1], linestyle=next(line_cycle), marker=next(marker_cycle),
                             linewidth=line_width, label=true_txt)
        pred_line = plt.plot(pred_traj[0], pred_traj[1], linestyle=next(line_cycle), marker=next(marker_cycle),
                             linewidth=line_width, label=pred_txt)
    ...
    plt.legend(fancybox=True, framealpha=1, shadow=True, borderpad=1, loc='upper left', prop={'size': 6},
               bbox_to_anchor=(1, 1))
    ...
    for file_index in range(len(plot_file_name)):
        ...
        for i in range(len(results)):
            ...
            if args.num_of_data is 0:  # test data visualization
                target_traj = plot_trajectories(results[i][0], results[i][1], results[i][2], results[i][3],
                                                results[i][4], name, figure_save_directory, args.min_traj,
                                                args.max_ped_ratio, results[i][5], [min_r, max_r, plot_offset],
                                                results[i][6])
            else:
                target_traj = plot_trajectories(results[i][0], results[i][1], results[i][2], results[i][3],
                                                results[i][4], name, figure_save_directory, args.min_traj,
